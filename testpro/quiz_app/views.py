...
import re
import json
from .models import Quiz, Choice , User_Quiz_Data
import os
...
# db 데이터 저장
def save_quiz_to_db(quiz_dict,user):
    #quiz_data_json = json.dumps(quiz_dict, ensure_ascii=False, indent=4)
    quiz = User_Quiz_Data.objects.create(
        ...
        if form.is_valid():
            ...
            try:
                ...
                if file_text:
                    ...
                    # 로그인된 사용자 정보를 request.user로 전달
                    save_quiz_to_db(quiz_dict, request.user)  # 여기에서 user=request.user
