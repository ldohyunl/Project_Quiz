...
class User_Quiz_Data(models.Model):
    ...
    score = models.IntegerField(null=True, blank=True)  # 사용자의 최종 점수 (기본값 없음)
    created_at = models.DateTimeField(auto_now_add=True)  # 생성 시간

    def __str__(self):
        return f"Quiz data for {self.user.username} at {self.created_at} (Score: {self.score if self.score is not None else 'N/A'})"